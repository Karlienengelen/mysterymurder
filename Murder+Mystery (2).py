...
#murder note and introduction texts
...
#make a definition to get the average sentence length of a text
def get_average_sentence_length(text):
    ...
    average_length = sum(sentences_length) / len(sentences_length)
    return average_length

#test to check my code
#print(get_average_sentence_length("kldsfb  ffdkdflkdfsl. ldfkljdfkjldfk e ksksk erkl kfkd . fkdkeh kdksf j e. "))
...
#define a class for saving the data about the problem
class TextSample():
    def __init__(self, text, author):
        self.raw_text = text
        self.author = author
        
        #save the average sentence length of a text
        self.average_sentence_length = get_average_sentence_length(text)
        
        #save the prepared text as prepared_text
    ...
    def __repr__(self):
        return "The author is {author} and the length is {length}".format(author=self.author, length=self.average_sentence_length)

#test to check my code
#print(TextSample(murder_note, "test"))
...
#create objects for each piece of text
...
#print lengths of each piece of text
...
def prepare_text(text):
    prepared_text = list(text.lower().replace(",", "").replace(".", "").replace("?", "").replace("!", "").split(" "))
    return prepared_text

#test to check my function: 
...
#function to create ngrams from the word lists from each note
def ngram_creator(text_list):
    adjacent_pairs = []
    for word in range(0, len(text_list) -1):
        adjacent_pairs.append(text_list[word] + " " + text_list[word + 1])
...
#function to compare frequencies in notes
...
#function to check the change between the average sentence lengths of two TextSamples

"""
#First try: didnt work. 
def percent_difference(TextSample1, TextSample2):
    numerator = abs(get_average_sentence_length(TextSample1)) - abs(get_average_sentence_length(TextSample1))
    denominator = (get_average_sentence_length(TextSample1) + get_average_sentence_length(TextSample1)) / 2
    return numerator / denominator
"""

def percent_difference(TextSample1, TextSample2):
    numerator = abs(TextSample1.average_sentence_length) - abs(TextSample2.average_sentence_length)
    denominator = (TextSample1.average_sentence_length + TextSample2.average_sentence_length) / 2
    return numerator / denominator

#test to check my code
#print(abs(murderer_sample.average_sentence_length))
#print(percent_difference(murderer_sample,lily_sample))
...
def find_text_similarity(TextSample1, TextSample2):
    ...
    word_count_similarity = abs(1 - (frequency_comparison(TextSample1.word_count_frequency, TextSample2.word_count_frequency)))
    #print(word_count_similarity)
    ngram_similarity = abs(1 - (frequency_comparison(TextSample1.ngram_frequency, TextSample2.ngram_frequency)))
    #print(ngram_similarity)
    return (sentence_length_similarity + word_count_similarity + ngram_similarity)/3

print(str(find_text_similarity(murderer_sample, lily_sample)*100)+"%") 
print(str(find_text_similarity(murderer_sample, myrtle_sample)*100)+"%") 
print(str(find_text_similarity(murderer_sample, gregg_sample)*100)+"%") 
...
print("The similarity score of Lily is 89.63%")
print("The similarity score of Myrtle is 78.66%")
print("The similarity score of Gregg is 81.53%")
